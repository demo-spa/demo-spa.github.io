...
const detailsTemplate = (recipe, likes, canLike, isLikedByUser, onLike, onComment, comments) => html`
<h2>${recipe.name}</h2>
<div>
    ${canLike ? html`<a href="javascript:void(0)" @click=${onLike}>Like</a>` : nothing}
    ${isLikedByUser ? html`<span class="liked">Liked</span>` : nothing}
...
async function loadRecipe(id, userId, onLike, onComment) {
    const [{ likes, isLikedByUser }, comments, recipe] = await Promise.all([
    ...
    const canLike = !isLikedByUser && !isOwner && isAuth;

    return detailsTemplate(recipe, likes, canLike, isLikedByUser, onLike, onComment, comments);
}